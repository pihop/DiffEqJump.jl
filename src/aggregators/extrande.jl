...
struct Extrande <: DiffEqJump.AbstractAggregatorAlgorithm end
...
mutable struct ExtrandeJumpAggregation{T,S,F1,F2,F3,F4,RNG} <: DiffEqJump.AbstractSSAJumpAggregator
    next_jump::Int
    prev_jump::Int
    next_jump_time::T
    end_time::T
...
function aggregate(aggregator::Extrande, u, p, t, end_time, constant_jumps,
    ...
    rates, affects! = DiffEqJump.get_jump_info_fwrappers(u, p, t, (constant_jumps..., bounded_va_jumps..., NullAffectJump))
    rbnds, wnds = get_va_jump_bound_info_fwrapper(u, p, t, (constant_jumps..., bounded_va_jumps...,NullAffectJump))
    DiffEqJump.build_jump_aggregation(ExtrandeJumpAggregation, u, p, t, end_time, ma_jumps,
                           rates, affects!, save_positions, rng; u=u, rate_bounds=rbnds, windows=wnds, kwargs...)
end

# set up a new simulation and calculate the first jump / jump time
function DiffEqJump.initialize!(p::ExtrandeJumpAggregation, integrator, u, params, t)
    p.end_time = integrator.sol.prob.tspan[2]
    DiffEqJump.generate_jumps!(p, integrator, u, params, t)
end

# execute one jump, changing the system state
@inline function DiffEqJump.execute_jumps!(p::ExtrandeJumpAggregation, integrator, u, params, t)
    # execute jump
    u = DiffEqJump.update_state!(p, integrator, u)
    nothing
end
